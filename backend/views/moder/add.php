...
$this->title = 'Добавление администратора системы';
                            ...
                            <div class="panel-body">
                                <?php if (isset($className["method"])) {
                                    foreach ($className["method"] as $key => $method) { ?>
                                        <?= $form->field($model, 'rules[' . $className["className"][0] . '_' . $method . '][]')->checkbox(['label' => $className["doc"][$key] ? $className["doc"][$key] : $className["className"][0] . " => " . $method]) ?>
                                    <?php }
                                } ?>
                            </div>
                        </div>
